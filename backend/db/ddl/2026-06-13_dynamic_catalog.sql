...
SELECT CASE
    ...
    WHEN text_blob ~ '\m(materials|natural resources|global resources|commodity stock|capital cycles|metals mining|mining producers|uranium|nuclear|copper|lithium|battery|energy infrastructure|mlp|world energy)\M'
        THEN 'Sector Equity'
    ...
    WHEN text_blob LIKE '%ex technology%'
      OR text_blob LIKE '%ex tech%'
        THEN 'Large Blend'
    WHEN text_blob ~ '\m(biotech|biotechnology|genome|genomics|medical technology|medical devices|health technology)\M'
        THEN 'Sector Equity'
