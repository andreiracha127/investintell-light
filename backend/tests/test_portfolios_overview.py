...
async def test_overview_etf_fund_ticker_uses_eod_and_live(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _install_stubs(
        monkeypatch,
...
def test_build_overview_nav_source_disables_live_overlay() -> None:
    rows, _ = build_overview(
    ...
    assert rows[0].fund_type == "mutual_fund"
