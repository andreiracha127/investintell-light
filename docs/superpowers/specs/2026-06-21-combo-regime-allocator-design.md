...
## 10. Productized name (2026-06-21, Sprint 5)

The owner chose the product name **Regime-Aware** for this allocator. The mapping is: internal objective literal **`regime_aware`** / user-facing display label **"Regime-Aware"** / research codename **COMBO**. Sprint 5 renamed the objective literal from the interim `"combo"` to `"regime_aware"` everywhere in code (the `Objective` literal in `app/schemas/builder.py`, the `run_optimize` dispatch branch and its local diagnostics variables in `app/services/portfolio_builder.py`, the scenario-path objective set, and the test files `test_builder_regime_aware*.py`). The research codename **COMBO** is retained UNCHANGED throughout this design doc and the Lean harness (`lean-research/TaaCvarSuite/main.py`, variant `COMBO`) — it is the historical research identity; `regime_aware` is only the productized API/UI name. Anywhere this spec says `"combo"` (e.g. §3.3, §7, Decision B), read it as the now-renamed `"regime_aware"` objective.

Sprint 5 also (a) added `regime_aware` to the builder's objective dropdown (`frontend/src/components/builder/assets.ts` `OBJECTIVES`, ranked mode only — it is scenario-based like `min_cvar`, so it is excluded from broad-universe mode), labelled "Regime-Aware (regime-conditioned)"; and (b) activated the vol/beta graduated-cap overlays for ANY universe by loading SPY as a SIGNAL series from `eod_prices` (via `_series.select_adj_close_rows`) over the optimization window — decoupled from traded-universe membership — with a degrade-safe fallback to the in-universe SPY proxy / flat cap when the read returns insufficient history (closing the Sprint-3 degradation where the overlays only fired when `equity:SPY` happened to be in the universe).
